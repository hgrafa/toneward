...
- `NotePlaybackContext.tsx` — `useNotePlayback`: the box-pattern note player. Owns a `NotePlayer` engine, the `tone`/`volume` settings (persisted to localStorage), its own output `deviceId` (discovery shared via `AudioDevicesContext`), and the transport (`play(id, pitches, direction)` / `stop` / `playing`). `playing` identifies the active card + direction so each card reflects play/stop state.
