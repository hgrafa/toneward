...
Before doing implementation work, run these steps **in order**:

### Step 0 — Claim the issue (run first, before anything else)

Extract the issue number from `$ARGUMENTS` and label it WIP so parallel sessions don't pick it up:
...
4. Inspect existing worktrees and git state:
...
**Always work in a git worktree, not in the main working tree.**

Invoke `superpowers:using-git-worktrees` to detect existing isolation and set up a workspace. Follow its instructions exactly. Key points for this project:

- Worktrees live in `.claude/worktrees/<slug>` (already git-ignored)
- Check for an existing worktree for this issue before creating one:
  ```bash
  git worktree list | grep "$ISSUE_NUMBER"
  ```
- If an existing worktree is found, cd into it and continue from there
- If not, create one:
  ```bash
  git worktree add .claude/worktrees/issue-$ISSUE_NUMBER-<short-slug> -b <branch-name>
  ```
- Branch naming follows `CLAUDE.md` conventions: `fix/...`, `feat/...`, `refactor/...`, etc.
- Do not work directly on `main`. Do not commit in the main working tree.

### Step 3 — Post a start checkpoint

Post a checkpoint comment to the issue (see Checkpoints section for format).
...
**Always work in a git worktree.** See Step 2 in Required operating mode above for the full setup sequence.

Branch names follow root `CLAUDE.md` conventions:
...
6. Do not commit superpower plan/spec files.

   * Files generated by `superpowers:writing-plans` or `superpowers:brainstorming` (e.g. `docs/superpowers/plans/`, `docs/superpowers/specs/`) are ephemeral session artifacts.
   * Decisions belong in commit messages, issue comments, and PR descriptions — not in committed files.
